...
lost-update interleaving. The rest becomes load-bearing at roadmap 08 (quota admission),
14 (the tap/IP allocator), and 17 (workers) — read it before writing any of those.
...
in-process mutable state that genuinely remains in memory: roadmap 14's tap/IP allocator,
workerd's local bookkeeping (roadmap 17), and any future cache. The *failure modes* (rule 1,
...
| --- | --- | --- | --- |
| firecracker backend (roadmap 14) | allocator `mu` | tap/IP allocations | reconciler-driven creates and destroys |
| worker (roadmap 17) | `Worker.mu` | local stats, in-flight operation table if one exists | run loop, heartbeat loop, HTTP API |
...
twice by name: roadmap 08's quota admission (count and insert in the same transaction) and
roadmap 17's placement reservation (`FOR UPDATE` on the host row before recording the
sandbox on it).
...
> or into the reserving transaction. Roadmaps 08 and 17 each carry a concurrency test
...
prod per roadmap 15 — the roadmaps skip the in-memory-store stage). Keep these properties:
...
quota check + insert in roadmap 08, capacity check + placement in roadmap 17), so the caller
...
- **Lost updates.** `-race` cannot see the resurrect-a-destroyed-sandbox sequence above.
  ...
  and is the template for the same test at future seams (roadmaps 08 and 17).
