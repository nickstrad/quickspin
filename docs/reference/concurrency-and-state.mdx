...
Part description of current code, part forward-looking guidance. The category-4 shape
below — a WHERE-guarded write that re-checks its assumption around slow Docker I/O — is
implemented as of roadmap 06: the reconciler uses it for every state transition, and
`TestReconcilerDoesNotResurrectDestroyedSandbox` (internal/reconciler) pins the
lost-update interleaving. The rest becomes load-bearing at roadmap 09 (quota admission),
17 (the tap/IP allocator), and 13 (hosts) — read it before writing any of those.
Adapted from an earlier codebase that guarded exactly this shape of state — "read a
record, do slow Docker I/O, write the result" — in real code.

One framing correction against the roadmaps: the sibling project held its records in
mutex-guarded maps, while Quickspin's roadmaps make the **database authoritative from roadmap
...
in-process mutable state that genuinely remains in memory: roadmap 17's tap/IP allocator,
hostd's local bookkeeping (roadmaps 08, 13), and any future cache. The *failure modes* (rule 1,
...
In the roadmaps, the fleet's records (sandboxes, events, hosts, capacity) live in the
...
| --- | --- | --- | --- |
| firecracker backend (roadmap 17) | allocator `mu` | tap/IP allocations | reconciler-driven creates and destroys |
| hostd (roadmaps 08, 13) | `Hostd.mu` | local stats, in-flight operation table if one exists | run loop, heartbeat loop, HTTP API |
...
never a SELECT in one transaction and an UPDATE in another. The roadmaps hit this shape
twice by name: roadmap 09's quota admission (count and insert in the same transaction) and
roadmap 13's placement reservation (`FOR UPDATE` on the host row before recording the
sandbox on it).
...
> or into the reserving transaction. Roadmaps 09 and 17 each carry a concurrency test
...
**SQL translation — the form the roadmaps actually use.** With the database authoritative
(roadmap 05 onward), the same guard becomes a conditional write that re-checks state in the
...
the reconciler treats it as "undo the side effect and stand down," not as success: its
`ActionCreate` destroys the container it just made when the write-back misses. This is
implemented (roadmap 06) and the interleaving is pinned by
`TestReconcilerDoesNotResurrectDestroyedSandbox` in internal/reconciler.
...
If scheduling or scoring does network I/O (fetching live stats from workers, roadmap 4), the
...
The records live behind the store interface roadmap 05 defines (SQLite locally, Postgres in
prod per roadmap 11 — the roadmaps skip the in-memory-store stage). Keep these properties:
...
invariants spanning more than one statement (state transition + event append in roadmap 06,
quota check + insert in roadmap 09, capacity check + placement in roadmap 13), so the caller
...
- **Lost updates.** `-race` cannot see the resurrect-a-destroyed-sandbox sequence above.
  Pin it by driving the exact interleaving — `TestReconcilerDoesNotResurrectDestroyedSandbox`
  (internal/reconciler) does this with a fake runtime whose `Create` blocks on a channel,
  and is the template for the same test at future seams (roadmaps 09 and 17).
