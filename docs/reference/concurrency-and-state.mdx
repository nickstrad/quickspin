...
as first built (roadmap 02) is single-goroutine and needs none of this. It becomes load-
bearing at roadmaps 05–06 (a control plane with a reconcile loop and an HTTP API touching
shared records) and roadmap 17 (workers). Read it before writing either. Adapted from a
earlier codebase that guarded exactly this shape of state — "read a record, do
...
the reconciler must treat it as "leave it alone," not as an error. Roadmap 06 mandates this
...
- **Lost updates.** `-race` cannot see the resurrect-a-destroyed-sandbox sequence above.
  Pin it by driving the exact interleaving — in the roadmaps this is roadmap 06's
