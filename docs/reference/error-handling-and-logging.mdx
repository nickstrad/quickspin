...
greeting to report on. It is adapted from an earlier codebase that carried these
...
| Package                  | Type                | Exists?         |
| ------------------------ | ------------------- | --------------- |
| `internal/runtime`       | `RuntimeError`      | roadmap 02         |
| `internal/events`        | `EventError`        | roadmap 06         |
| `internal/controlplane`  | `ControlPlaneError` | future (roadmap 05) |
| `internal/worker`        | `WorkerError`       | future (roadmap 13) |

The envelope itself is defined once, in `internal/errs`, as a generic struct whose type
parameter is a phantom tag naming the owning package:
...
work through the whole chain. `internal/errs` imports nothing from `internal/`, since
every package that owns an envelope imports it.

Each package's `errors.go` then declares a tag, aliases the instantiation, and re-exports
`E` and `Wrap` so its call sites and its callers are unchanged:
...
The tag is what keeps the errors self-identifying: `errs.Error[runtimeTag]` and
`errs.Error[storeTag]` are unrelated types, so a package's `errors.As` and type switches
still match only its own envelope. The type alias (`=`, not a defined type) is what makes
`*RuntimeError` the same type as `*errs.Error[runtimeTag]`, so the shared methods apply
without a per-package re-declaration.

Sentinels stay in the package that owns them, and so does any package-specific helper —
`httpapi.OpOf`, for example. Only the envelope is shared.

One cost of the move: `E` captures `debug.Stack()` from inside `internal/errs`, so the
stack carries two extra frames (`errs.E` and the package's `E`) above the origin call
site.

### Composing with the sentinel errors

Roadmap 02 commits `internal/runtime` to sentinel errors (`ErrNotFound`, `ErrImageMissing`)
...
The HTTP case arrives with the first process boundaries: `sbxd` (roadmap 07) is the
first separate process, and the host agent (roadmap 08) is the second. Either way, a Go
error never crosses the wire — the remote side serializes the shared error DTO, and the
client (`sbxd.Client`, the plane's worker client) logs what the DTO said and mints a
fresh error of its own package's type rather than pretending to wrap an error it does not
have. The reconciler loop (roadmap 06) is the other case — a top-of-goroutine loop that logs
...
When the control plane grows an HTTP API (roadmap 05) and later clients (the guest client
in roadmap 07, the worker client in roadmap 13), a shared `internal/httpapi` helper should own
...
roadmap 05 creates the package for exactly this reason.
...
- `Stack` is captured at every origin but nothing reads it unless a log site emits it.
  Either surface it at `Debug` (a `LogValue()` on `errs.Error`, which every package would
  inherit at once) or drop the field; it costs a `debug.Stack()` per error for no payoff
...
- Sentinel vs struct is resolved in the roadmaps (see roadmap 02): the sentinels are the public
  contract callers switch on; the struct is the diagnostic envelope carrying them in
  `Err`. The composition is pinned by a conformance test in `internal/errs`
  (`TestErrorPreservesSentinelThroughWraps`) so the envelope can never silently swallow a
  sentinel it was handed — one test now covers every package that aliases it.
