...
| `internal/worker`        | `WorkerError`       | future (roadmap 17) |
...
The HTTP case arrives earlier than the worker split: the guest agent (roadmap 07) is the
first separate process, and the worker daemon (roadmap 17) is the second. Either way, a Go
...
in roadmap 07, the worker client in roadmap 17), a shared `internal/httpapi` helper should own
