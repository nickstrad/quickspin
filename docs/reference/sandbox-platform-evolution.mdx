# Sandbox Platform Evolution

This document maps one route from a local sandbox runtime to scalable agent sandbox
infrastructure. It is forward-looking reference material, not a specification or an
...
The stages are now instantiated as concrete platform roadmaps under `docs/plans/`; where the
two disagree, the roadmaps are authoritative. Rough mapping: stage 1 → roadmaps 01–04, stage 2
→ roadmaps 05–08, stage 3 → roadmaps 09–10 (plus Python), stage 4 → roadmap 17 (which deliberately
defers this stage's ownership leases — see that roadmap's known-gaps note), stage 5 → roadmap
11, stage 6 → roadmaps 13–14, stage 7 → roadmaps 16 and 18. Refer to stages by roadmap number in
other documents; bare stage numbers have already been mistaken for roadmap numbers once.
...
Add only enough state to establish control-plane behavior:
...
Docker backend to strengthen lower-level isolation:
